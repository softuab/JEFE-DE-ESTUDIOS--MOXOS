...
<br>
<form  name=forma action="<c:url value="/estudianteNuevo/registrarPersonaEstudiante.fautapo"/>" method="POST">
  <input type="hidden" name="gestion" value="<c:out value='${gestion}'/>">
  <input type="hidden" name="periodo" value="<c:out value='${periodo}'/>">
    ...
    <tr>
      <td>
        <table class="tabla">
	      ...
	      <input type="text" name="paterno" size="20" title="Apellido Paterno"   onkeyup="fmayus();" value="<c:out value='${bPst.paterno}'/>" />
	    </td>	
	   <td class="etiqueta3">2do. Apellido<br>
	        <input type="text" name="materno" size="20" title="Apellido Materno" onkeyup="fmayus();"  value="<c:out value='${bPst.materno}'/>"/>
	   </td>	
	   <td class="etiqueta3">Nombres <font color="red" >(*)</font></<br>
	       <input type="text" name="nombres" size="20" title="Nombres"  onkeyup="fmayus();"  value="<c:out value='${bPst.nombres}'/>" />
	   </td> 
	  </tr>   
	  <tr> 
	    <td class="etiqueta3"> 
	        <label><font size="1px" color="#003366"><b>DIP <font color="red" >(*)</font> </b></font><br></label>
		 <input type="text"  name="dip" title="DIP" size="10" value="<c:out value='${bPst.dip}'/>"/>
	    </td>
	  ...
	  <tr> 
	    ...
	    <td class="etiqueta3">Titulo Adquirido<br> 
	      <input type="text"  name="titulo"  title="" size="10"/>
	    </td>
	    <td class="etiqueta3"> 
	    </td>
	  <tr>
	  ...
	  <tr> 
	   ...
	   <td class="etiqueta3">Nro. Seguro M&eacute;dico<br> 
		 <input type="text"  name="nro_seguro_medico"  title="Nro. Seguro Medico" size="10"/>
	   </td>
	  <tr>
	  </table>
	</td>
	    ...
	    <td class="etiqueta4">
	      ...
	      <td>
            ...
            <script>
              ...
              padre_hijo[h] = new Array ("plan", "id_programa", "<c:out value = "${bPst.id_plan}"/>");
              combo[h] = new Array();
              combo[h][0] = new Array("0", "- Elija una opcion -", "");
              <c:forEach var = "cod" items = "${lPlanesActual}" varStatus = "planc">
                Aplanes[<c:out value = "${cod.plan}"/>] = '<c:out value = "${cod.plan}"/>';
                combo[h][<c:out value = "${planc.count}"/>] = new Array("<c:out value = "${cod.plan}"/>", "<c:out value = "${cod.plan}"/> - <c:out value = "${cod.tipo_grado}"/>", "<c:out value = "${cod.id_programa}"/>");
              </c:forEach>
              h++;
            </script> 
	  </tr>
	  ...
	  <tr>
	    ...
	    <td>
              <select name="id_tipo_admision">
                ...
                <option value='<c:out value="${listaA.id_tipo_admision}"/>' <c:if test="${bPst.id_tipo_admision ==  listaA.id_tipo_admision}"> selected </c:if>>
