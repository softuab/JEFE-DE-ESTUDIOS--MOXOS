...
<html lang="es">
    ...
    <body class="app sidebar-mini">
        <main class="app-content3">
            <div class="app-title">
                <div>
                    <h1><i class="fa fa-dashboard"></i><i class="far fa-address-card"></i>&nbsp;Ver Actas Por Fases</h1>
                    <p>${usuario}</p>
                </div>
            </div>
            <div class="row">
                <div class="col-sm-12">
                    <div class="tile">
                        <form name='forma' method="post" action='<c:url value='/planes/verPlanEstudios.fautapo'/>'>
                            ...
                            <c:if test="${fn:length(acceso.listaFacultades) > 0}">
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-8">
                                        <select name="id_facultad" id="id_programa" class="form-control" onChange="poblar(this.name, this.options[this.selectedIndex].value)"></select>
                                    </div>
                                </div>
                            </c:if>   
                                    ...
                                    <div class="col-md-8">
                                        <select name="id_plan" id="id_plan"  class="form-control" required>
                                            ...
                                            <c:forEach var="plan" items="${acceso.listaPlanes}" varStatus="pln">
                                                <option value="<c:out value="${plan.id_plan}"/>-<c:out value="${plan.id_prg_plan}"/>" ><c:out value="${plan.tipo_grado}"/> - <c:out value="${plan.id_plan}"/></option>
                                            </c:forEach>
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="tile-footer">
                                <div class="row">
                                    <div class="col-md-8 col-md-offset-3">
                                        <button class="btn btn-primary" type='submit' value='Siguiente'><i class="fa fa-fw fa-lg fa-check-circle"></i>Siguiente</button>&nbsp;&nbsp;&nbsp;<a class="btn btn-secondary"  href="<c:url value='/planes/verPlanesEstudiosEntrada.fautapo'/>"><i class="fa fa-fw fa-lg fa-times-circle"></i>Cancelar</a>
                                    </div>
                                </div>
                            </div>
                        </form>
