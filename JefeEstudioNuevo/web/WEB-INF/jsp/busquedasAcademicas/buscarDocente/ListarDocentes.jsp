...
<html lang="es">
    ...
    <body>
        <main class="app-content3">
            <div class="app-title">
                <div>
                    <h1><i class="fa fa-dashboard"></i><i class="far fa-address-card"></i>&nbsp;Buscar Docentes</h1>
                    <p>${usuario}</p>
                </div>
            </div>
            <div class="row">
                                    ...
                                    <tbody>
                                        <c:forEach var="datos" items="${lDocentes}" varStatus="contador">
                                            <tr <c:if test="${(contador.count mod 2) == 0}"></c:if>>
                                                ...
                                                <td><form name=forma<c:out value="${contador.count}"/> method="POST" action="<c:url value="/buscarDocente1.fautapo"/>">                                           
                                                        <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>    
                                                        <input type="hidden" name="id_docente" value='<c:out value="${datos.id_docente}"/>' >
                                                    </form>
                                                    <a  class="btn btn-primary" href='javascript: document.forma<c:out value="${contador.count}"/>.submit();' > Ver asignaci&oacute;n </a></td>
                                            </tr>
                                            </form>
                                        </c:forEach>
                                    </tbody>
