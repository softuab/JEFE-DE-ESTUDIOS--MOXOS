...
<form name="fvolver" action="<c:url value='/estudianteAntiguoPost/entrada.fautapo'/>" method="post">
...
<br>
<form  name=forma action="<c:url value="/estudianteAntiguoPost/registrarTiposDocumentosPersona.fautapo"/>" method="POST">
  <table class="formulario">
    <tr>
	  ...
	  <!--
	  <tr>
            <td class="etiqueta4">Desbloquear ? ::</td>
	    <td>
	      SI<input type="radio" name="habilitar"   value="Si" <c:if test="${buscarEst.id_estado == 'B'}"> checked </c:if> > &nbsp;
              NO <input type="radio" name="habilitar"  value="No" <c:if test="${buscarEst.id_estado == 'A'}"> checked</c:if> >
	    </td>
	  </tr>
	  -->
	</table>
	 ...
	 <table class="tabla" witdh="50%">
	   ...
	   <c:forEach var="lDocumento" items="${lPrsDocumentosClasificacion}" varStatus="contadorClas">
		 ...
		 <c:if test="${lDocumento.compromiso == true}">
	       ...
	       <td><c:out value="${gestion_matricula}"/>-<c:out value="${periodo_matricula}"/>  </td> 
	       <td nowrap> 
	...
	<input type="hidden" name="id_tramite"     value="<c:out value="${id_tramite}"/>">
	<input type="hidden" name="gestion"        value="<c:out value="${gestion_matricula}"/>">
	<input type="hidden" name="periodo"        value="<c:out value="${periodo_matricula}"/>">
	<input type="hidden" name="titulo"         value="<c:out value="${titulo}"/>">
      </td>
    </tr>
  </table>
