...
public class Funciones implements Serializable {
    ...
    public int matriz_etiquetas(String sql, String lista, int desde, String sdesde, int numeros, String etiqueta) {
        ...
        for (int i = 0; i < v; i++) {
            abm = (Abm) valores.get(i);
            String valor1[] = abm.getValores().split(",");
            System.out.println(valor1.length + " - " + i);
            for (int j = 0; j < valor1.length; j++) {
                datos1[i][j] = valor1[j].replaceAll("#~~~#", ",");
                if (j < numeros) {
                    datos[i][j] = "<td align=left>" + valor1[j].replaceAll("#~~~#", ",") + "</td>";
    ...
    public int matriz_etiquetas_subtotales(String sql, String lista, int desde, String sdesde, int numeros, String etiqueta) {
        ...
        for (int i = 0; i < v; i++) {
            ...
            for (int j = 0; j < valor1.length; j++) {
                String a = "";
                if (valor1[0].equals("2")) {
                    a = " height=15 style=\"border-top: 2px solid #000000; border-bottom: 2px solid #000000;\"";
                }
                if (j < numeros) {
    ...
    public void pies(String cadena) {
        ...
        if (valores.size() == 1) {
            modelo.put("titulo", ((Abm) valores.get(0)).getValores());
        }
        ...
        valores = this.mi.getEjecutarListado(abm);
        if (valores.size() == 1) {
            modelo.put("glosa", ((Abm) valores.get(0)).getValores());
        }
        ...
        valores = this.mi.getEjecutarListado(abm);
        if (valores.size() == 1) {
            modelo.put("firmas", ((Abm) valores.get(0)).getValores());
        }
    }
...
// otros
    ...
    // Combos
    public void combo_unidad() {
        combo("SELECT u.id_unidad, replace(substring(u.unidad for 50), ',', '#~~~#') FROM unidades u, _usr_unidades a, _usr_roles b WHERE u.id_unidad=a.id_unidad AND u.id_tipo_calificacion=1 AND a.id_usr_rol = b.id_usr_rol AND b.id_usuario=" + ((Clientes) request.getSession().getAttribute("__sess_cliente")).getId_usuario(), "lunidad");
    }

    public void combo_apertura() {
        combo("SELECT p.id_apertura_programatica, replace((codigo_apertura_programatica ||' - '|| apertura_programatica), ',', '#~~~#'), id_unidad FROM pr_aperturas_programaticas p, pr_und_aperturas a WHERE p.id_apertura_programatica = a.id_apertura_programatica order by codigo_apertura_programatica", "lapertura");
    }

    public void combo_gestion() {
        combo("SELECT valor|| '-01-01', now()::date FROM _parametros WHERE sistema='financiero' and campo='gestion'", "lgestion");
    }

    public void combo_periodo_contable() {
        combo("SELECT DISTINCT  id_periodo_contable, substring(periodo_contable for 50) as periodo_contable FROM periodos_contables ORDER BY periodo_contable", "lperiodo_contable");
    }
    ...
    public void combo_cuenta_bancaria() {
        combo("select id_cuenta_bancaria, replace(substring((numero ||' - '|| nombre) for 50), ',', '#~~~#'),id_unidad from ts_cuentas_bancarias order by numero", "lcuenta_bancaria");
    }

    public void combo_entidad_transferencia() {
        combo("SELECT DISTINCT et.id_entidad_transferencia, replace(substring(codigo_entidad_transferencia || ' - ' || entidad_transferencia for 50), ',', '#~~~#')  FROM entidades_transferencias et, (SELECT DISTINCT id_entidad_transferencia FROM ct_cmp_detalles) c WHERE et.id_entidad_transferencia = c.id_entidad_transferencia ORDER BY replace(substring(codigo_entidad_transferencia || ' - ' || entidad_transferencia for 50), ',', '#~~~#')", "lentidad_transferencia");
    }

    public void combo_fuente_financiamiento() {
        combo("SELECT DISTINCT fo.id_fuente_financiamiento, codigo_fuente_financiamiento || ' - ' || replace(fuente_financiamiento, ',', '#~~~#') FROM fuentes_financiamientos ff, (SELECT DISTINCT id_fuente_organismo FROM ct_cmp_detalles) c, fnt_organismos fo WHERE ff.id_fuente_financiamiento = fo.id_fuente_financiamiento AND c.id_fuente_organismo = fo.id_fuente_organismo ORDER BY codigo_fuente_financiamiento || ' - ' || replace(fuente_financiamiento, ',', '#~~~#')", "lfuente_financiamiento");
    }

    public void combo_organismo_financiador() {
        combo("SELECT DISTINCT fo.id_fuente_organismo, codigo_organismo_financiador || ' - ' || replace(organismo_financiador, ',', '#~~~#'), id_fuente_financiamiento FROM organismos_financiadores ff, (SELECT DISTINCT id_fuente_organismo FROM ct_cmp_detalles) c, fnt_organismos fo WHERE ff.id_organismo_financiador = fo.id_organismo_financiador AND c.id_fuente_organismo = fo.id_fuente_organismo ORDER BY codigo_organismo_financiador || ' - ' || replace(organismo_financiador, ',', '#~~~#')", "lorganismo_financiador");
    }

    public void combo_almacen_activos() {
        combo("SELECT id_almacen, replace(substring(almacen for 50), ',', '#~~~#') as almacen FROM almacenes al WHERE id_tipo_almacen =3  AND id_almacen not in ( SELECT DISTINCT id_almacen_padre FROM almacenes) ORDER BY almacen", "lalmacen");
    }

    public void combo_tipo_baja() {
        combo("SELECT DISTINCT  id_tipo_baja, replace(substring(tipo_baja for 50), ',', '#~~~#') as tipo_baja FROM act_tipos_bajas ORDER BY tipo_baja", "ltipo_baja");
    }

    public void combo_casa_comercial() {
        combo("SELECT DISTINCT  id_casa_comercial, replace(substring(casa_comercial for 50), ',', '#~~~#') as casa_comercial FROM ss_casas_comerciales ORDER BY casa_comercial", "lcasa_comercial");
    }

    public void combo_tipo_restringido() {
        combo("SELECT DISTINCT  id_tipo_restringido, replace(substring(tipo_restringido for 50), ',', '#~~~#') as tipo_restringido FROM act_tipos_restringidos ORDER BY tipo_restringido", "ltipo_restringido");
    }

    public void combo_tipo_tipo_estado_activo() {
        combo("SELECT DISTINCT  id_tipo_estado_activo, replace(substring(tipo_estado_activo for 50), ',', '#~~~#') as tipo_estado_activo FROM act_tipos_estados_activos ORDER BY tipo_estado_activo", "ltipo_estado_activo");
    }
